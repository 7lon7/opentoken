...
export default function PricingPage() {
  return (
    <div style={{ background: "#fff" }}>
      {/* Header */}
      <div style={{ padding: "clamp(44px, 7vw, 64px) clamp(16px, 4vw, 28px) clamp(28px, 5vw, 48px)", background: "linear-gradient(180deg, #F0FDFA 0%, #fff 100%)", borderBottom: "1px solid #E5E7EB", textAlign: "center" }}>
      ...
      <div style={{ maxWidth: 1120, margin: "0 auto", padding: "clamp(34px, 6vw, 56px) clamp(16px, 4vw, 28px)" }}>
        {/* Pricing cards */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: "clamp(14px, 2vw, 24px)" }} className="pricing-pg-grid">
          {plans.map(({ name, price, period, desc, badge, features, highlight, cta }) => {
            ...
            return (
              <div key={name} style={{
                padding: "clamp(24px, 3.2vw, 36px) clamp(18px, 3vw, 30px)", borderRadius: 20,
              ...
              <div style={{ marginBottom: "clamp(20px, 3vw, 28px)" }}>
                ...
                <div style={{ display: "flex", alignItems: "flex-end", gap: 2, marginBottom: 10 }}>
                  <span style={{ fontSize: "clamp(2rem, 5vw, 2.6rem)", fontWeight: 800, color: highlight ? "#0D9488" : "#111827", letterSpacing: "-0.04em", lineHeight: 1 }}>{price}</span>
        ...
        {/* Key advantages */}
        <div style={{ marginTop: "clamp(38px, 6vw, 64px)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: "clamp(12px, 1.8vw, 16px)" }} className="adv-grid">
            {[
              ...
              <div key={title} style={{ padding: "clamp(18px, 2.8vw, 24px)", borderRadius: 16, border: "1px solid #E5E7EB", background: "#FAFAFA" }}>
      ...
      <style>{`
        @media (max-width: 900px) {
          .pricing-pg-grid { max-width: 620px; margin: 0 auto; }
